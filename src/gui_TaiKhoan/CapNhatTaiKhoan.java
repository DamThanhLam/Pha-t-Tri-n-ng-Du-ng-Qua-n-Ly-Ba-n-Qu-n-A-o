...
public class CapNhatTaiKhoan extends JPanel implements ActionListener, MouseListener{
	...
	/*
	 * Chưa hoàn thiện cập nhật và thêm
	 * */
	...
	private void initial() {
		...
		listNV = nhanVien_DAO.getAllNhanVienChuaCoTaiKhoan();
		listTK = taiKhoan_DAO.getAllTaiKhoan();
		
		loadTableNhanVien(listNV);
		loadTableTaiKhoan(listTK);
	}

	private void loadTableTaiKhoan(List<TaiKhoan> listTK) {
		for (TaiKhoan taiKhoan : listTK) {
			NhanVien nv = taiKhoan.getNhanVien();
			String row[] = {nv.getMaNV(),nv.getTen(),nv.getSoDienThoai(),nv.getChucVu().equals(utils.Contains.NHAN_VIEN_QUAN_LY) ?"Nhân Viên Quản Lý":"Nhân Viên Bán Hàng",taiKhoan.getTaiKhoan(),taiKhoan.getMatKhau()};
	...
	public void actionPerformed(ActionEvent e) {
		...
		if(o.equals(btnTimKiem)) {
			...
			for (NhanVien nhanVien : listNV) {
				...
				if(maNV.contains(nhanVien.getMaNV()) && tenNV.contains(nhanVien.getTen()) 
			...
			String maNV = txtTaiKhoanMaNhanVien.getText().toString().trim();
			if(maNV.equals("")) {
